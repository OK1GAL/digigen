 ...
 *
 */
int Si5351_init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
	...
	// Start I2C comms
	i2c_init(SI5351_I2C_PORT, 400 * 1000);
    gpio_set_function(SI5351_I2C_SDA, GPIO_FUNC_I2C);
    gpio_set_function(SI5351_I2C_SCL, GPIO_FUNC_I2C);
    gpio_pull_up(SI5351_I2C_SDA);
    gpio_pull_up(SI5351_I2C_SCL);
...
void set_pll(uint64_t pll_freq, enum si5351_pll target_pll)
{
  ...
  uint8_t *params = malloc(20*sizeof(uint8_t));
  uint8_t i = 0;
  uint8_t temp;
...
uint8_t si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	i2c_write_blocking(SI5351_I2C_PORT, SI5351_BUS_BASE_ADDR, buf, bytes+1, false);
...
uint8_t si5351_write(uint8_t addr, uint8_t data)
{
	...
	i2c_write_blocking(SI5351_I2C_PORT, SI5351_BUS_BASE_ADDR, buf, 2, false);
}
...
uint8_t si5351_read(uint8_t addr)
{
	...
	// 	reg_val = Wire.read();
	// }
	i2c_write_blocking(SI5351_I2C_PORT, SI5351_BUS_BASE_ADDR, &addr, 1, true);
	i2c_read_blocking(SI5351_I2C_PORT, SI5351_BUS_BASE_ADDR, reg_val, 1, false);
